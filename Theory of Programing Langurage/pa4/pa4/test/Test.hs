...
unit2 sc =
  testGroup
    ...
    [ scoreTest (sqSum, [], 0, 1, "sqSum 1"),
      ...
      scoreTest (uncurry sepConcat, ("a", ["", ""]), "a", 1, "sepConcat 1"),
...
scoreTest' sc (f, x, expR, points, name) =
  testCase name $ do
    ...
    let result = f x  -- Compute the result of f x
    putStrLn $ "Expected result: " ++ show expR ++ name
    putStrLn $ "Actual result: " ++ show result ++ name
    if f x == expR
      then updateCurrent sc points
      else do
        assertFailure "Wrong Result"
