...
digitsOfInt 0 = []
digitsOfInt n
  | n <= 0    = []  -- Return an empty list for non-positive n
...
additivePersistence n
  | n < 10    = 0  -- If n is already a single digit, no further additions are needed.
...
mulByDigit i l = removeZero res
  where
    (_, res)         = foldRight f base (0:l)
    f x (carry, sum) = let prod = x * i + carry
                       in (prod `div` 10, prod `mod` 10 : sum)
    base             = (0, [])
...
bigMul l1 l2 = res
  where
    (_, res)   = foldRight f base args
    f x (z, p) = let t = mulByDigit x l1
                     h = foldl (\acc x -> acc * 10 + x) 0 t
                     k = h * z
                     e = digits k
                 in (z * 10, bigAdd p e)
    base       = (1, [])
