...
const ls_size = ls => quick.fold_left((size) => size + 1, 0, ls); /** </FILL-IN> **/
...
const sum_sqrs = ls => quick.fold_left((acc, x) => acc + x*x, 0, ls);
...
const ls_evens = ls => quick.filter((x) => x % 2 == 0, ls)
const ls_odds  = ls => quick.filter((x) => x % 2 != 0, ls)
...
const reverse = (ls, result = List()) => {
  if (ls.size === 0) {
    return result;
  }

  return reverse(ls.rest(), result.unshift(ls.first()));
};
...
const reverse2 = ls => quick.fold_left((acc, x) => acc.unshift(x), List([]), ls);
...
{
  return quick.map(str => List([str, str === reverse_str(str)]), ls);
}
