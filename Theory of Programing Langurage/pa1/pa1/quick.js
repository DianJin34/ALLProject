...
const fold_left = function (f, base, ls) {
  ...
  const head = ls._tail.array[0];
  const tail = new List(ls._tail.array.slice(1));

  // console.log(base);
  return fold_left(f, f(base, head), tail);
};
...
const map = (g, ls) =>
{
  return fold_left((acc, current) => acc.push(g(current)), (List([])), ls);
}
...
const quicksort = function(ls) {
  ...
  const smaller = partition((element) => element <= pivot, rest).get(0);
  const larger = partition((element) => element > pivot, rest).get(0);
  
  // Recursively sort the partitions and combine them with the pivot
  return quicksort(smaller).push(pivot).concat(quicksort(larger));
  /** </FILL-IN> **/
};
